...
const handlePlayerMatchPairs = (playerRequest, playerMatch, gameState) => {
    if (playerRequest.player === 1) {
        ...
        gameState.player1Hand = gameState.player1Hand.filter(card => card.id !== playerRequest.card.id);
        gameState.player2Hand = gameState.player2Hand.filter(card => card.id !== playerMatch.card.id);
    }
    if (playerRequest.player === 2) {
        gameState.player2Pairs.push(playerRequest.card, playerMatch.card);
        gameState.player2Hand = gameState.player2Hand.filter(card => card.id !== playerRequest.card.id);
        gameState.player1Hand = gameState.player1Hand.filter(card => card.id !== playerMatch.card.id);
    }
    return gameState;
};
